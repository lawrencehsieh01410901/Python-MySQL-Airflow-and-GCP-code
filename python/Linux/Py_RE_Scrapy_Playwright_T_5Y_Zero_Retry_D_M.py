...
##======>>>1. IMPORT
LOG_FILE_NAME = f"INTERIOR_WEB_RE_TRANSACTION_{datetime.now().strftime('%Y_%m_%d_%H%M%S')}_LOG.txt"##2025-08-12，以_LOG.TXT做結尾，好讓之後的HOUSE KEEPING可以做LOG檔整理
...
dynamic_trans_ddl = mysql_params.get('dynamic_trans').replace("{formatted_time}", formatted_time)
dynamic_check_list_ddl= mysql_params.get('dynamic_check_list').replace("{formatted_time}", formatted_time)
pattern = r"CREATE TABLE\s+([^\s(]+)"
matches1 = re.findall(pattern, dynamic_trans_ddl, re.IGNORECASE)
matches2 = re.findall(pattern, dynamic_check_list_ddl, re.IGNORECASE)
if matches1:
    dynamic_trans = "".join(matches1)
if matches2:
    dynamic_check_list = "".join(matches2)

##2025-08-11，由於是同年，故不需要設定前幾年的值
...
def py_mysql_dynamic_ddl(conn, dynamic_trans: str, dynamic_trans_ddl: str, dynamic_check_list: str, dynamic_check_list_ddl: str) -> str:
    """Dynamically Create Table For Diff Day By Month Data, 01/11/21"""
    cond_py_mysql_dynamic_ddl = ""
    try:
        with conn.cursor() as cursor:
            ##DROP TABLE IF EXISTS
            check_dynamic_trans_tb_exists = cursor.execute(f"SELECT TABLE_NAME FROM information_schema.tables where TABLE_NAME = '{dynamic_trans}';")
            if check_dynamic_trans_tb_exists > 0:
                #drop_table = f"DROP TABLE IF EXISTS {dynamic_trans};"
                #cursor.execute(drop_table)
                #conn.commit()##確定生效
                cond_py_mysql_dynamic_ddl = f"DYNAMIC TABLE: {dynamic_trans} EXISTS!!! PLEASE CHECK FOR VERIFICATION###\n"
            ##CREATE DYNAMIC TB
            elif check_dynamic_trans_tb_exists == 0:
                dynamic_ddl = dynamic_trans_ddl
                cursor.execute(dynamic_ddl)
                conn.commit()##確定生效
                now = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                cond_py_mysql_dynamic_ddl += f"Dynamic TB: {dynamic_trans} created, at: {now}###\n"
            check_dynamic_check_list_tb_exists = cursor.execute(f"SELECT TABLE_NAME FROM information_schema.tables where TABLE_NAME = '{dynamic_check_list}';")
            if check_dynamic_check_list_tb_exists > 0:
                #drop_table = f"DROP TABLE IF EXISTS {dynamic_check_list};"
                #cursor.execute(drop_table)
                #conn.commit()##確定生效
                cond_py_mysql_dynamic_ddl += f"DYNAMIC TABLE: {dynamic_trans} EXISTS!!! PLEASE CHECK FOR VERIFICATION###\n"
            elif check_dynamic_check_list_tb_exists == 0:
                dynamic_ddl = dynamic_check_list_ddl
                cursor.execute(dynamic_ddl)
                conn.commit()##確定生效
                now = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                cond_py_mysql_dynamic_ddl += f"Dynamic TB: {dynamic_trans} created, at: {now}###\n"
    except Exception as ex_py_mysql_dynamic_ddl:
        err_detail = tb.format_exc()
        cond_py_mysql_dynamic_ddl = f"Error During Func: py_mysql_dynamic_ddl, Type: {type(ex_py_mysql_dynamic_ddl).__name__}, Error Message: {ex_py_mysql_dynamic_ddl}, Trace Back: {err_detail}>>>\n"
    return cond_py_mysql_dynamic_ddl
cond_py_mysql_dynamic_ddl = py_mysql_dynamic_ddl(mysql_conn, dynamic_trans, dynamic_trans_ddl, dynamic_check_list, dynamic_check_list_ddl)
program_begin_log += cond_py_mysql_dynamic_ddl
##======>>>3. DATABASE CONNECTION & SEARCH BAR PARAMS
...
async def async_open_website(async_p, url: str, wait_open_time: int = 5000) -> Tuple[str, Optional[Browser], Optional[Frame]]:
    ...
    try:                      ##Here, Assign Chromium as our Browser
        browser = await async_p.chromium.launch(headless=True, ##<<< Here, In Linux
...
async def get_last_page_num(browser: Browser, driver: Frame) -> Tuple[str, int]:
    ...
    try:  ##Since Time Duration is Min ~ Current, Need Time to Loading
        ...
        max_retry = 4##Retry 5 Times, Total Timeout: 150秒
        for i in range(max_retry):
            ...
            if re.search(r'查詢結果\s*[:：]\s*([\d,]+)\s*筆', info_text):
                ...
                await driver.wait_for_timeout(1500)##Wait 1.5 Secs 2025-08-11 hotfix!
        ##======>>>
...
async def main():
    try:
        async with async_playwright() as async_p:
            ...
            condition_2_1, count = await my_sql_check_list_count(mysql_conn, dynamic_check_list)
            await log_record(condition_2_1)

            if count == 0: ##代表第一次跑，才要重新匯入check list
                condition_2_2 = await mysql_insert_check_list(mysql_conn, city_district_data, dynamic_check_list)
            ...
            cond_get_incomplete_city_district, city_district_data = await get_incomplete_city_district(mysql_conn, dynamic_check_list)
            ...
            for data in city_district_data:
                ...
                if get_lastpage_num is None:
                    ...
                    update_log = await mysql_update_check_list(mysql_conn, dynamic_trans, dynamic_check_list, city_district_data[data]['city'], city_district_data[data]['district'])
                ...
                ##2025-07-11 HOTFIX======>>>
                    ...
                    insert_log = await mysql_insert_transaction(mysql_conn, full_batch_data,dynamic_trans, city_district_data[data]['city'], city_district_data[data]['district'])
